...
draft: false
---
import Turkey from '@components/turkey/Turkey.svelte'
...
![Original links page screenshot](./links-original.png)
...
![Links page with scrolling turkey](./scrolling-turkey.gif)
...
function createTurkeyContainer() {  // [!code ++]
    const turkeyContainer = document.createElement('div'); // [!code ++]
    turkeyContainer.setAttribute('class', 'turkey'); // [!code ++]
    document.body.appendChild(turkeyContainer); // [!code ++]
    adjustTurkeyAnimation()// [!code ++]
} // [!code ++]

window.addEventListener('load', adjustTurkeyAnimation); // [!code --]
window.addEventListener('load', createTurkeyContainer); // [!code ++]
window.addEventListener('resize', adjustTurkeyAnimation);
```
...
```Nunjucks
{% include "assets/turkey/turkey.html" %}
```

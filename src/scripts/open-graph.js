// borrowed heavily from https://www.emgoto.com/astro-social-card/

import fs from 'fs';
import { join } from 'path';
import puppeteer from 'puppeteer';

const CARD_WIDTH = 800;
const CARD_HEIGHT = 400;

const CONTENT_DIRS = [
    { path: 'src/content/blog', type: 'blog' },
    { path: 'src/content/projects', type: 'project' },
];
const OUTPUT_FOLDER = 'public/social-cards';

const BASE_URL = 'http://localhost:4321';
const TIMEOUT = 1000;

const LIMIT = 100;

async function timeoutFn(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}

function getSlugs(contentPath) {
    return fs
        .readdirSync(contentPath, { withFileTypes: true })
        .filter((path) => {
            const isMarkdownFile = (path) => /\.(md|mdx)$/i.test(path);
            return path.isDirectory()
                ? ['index.md', 'index.mdx'].some((file) =>
                      fs.existsSync(join(contentPath, path.name, file)),
                  )
                : path.isFile() && isMarkdownFile(path.name);
        })
        .map((path) => path.name.replace(/\.(md|mdx)$/i, ''));
}

const generateSocialCards = async (limit = LIMIT) => {
    const outputPath = join(process.cwd(), OUTPUT_FOLDER);

    const browser = await puppeteer.launch({
        args: ['--no-sandbox', '--disable-setuid-sandbox'],
    });

    for (const { path: contentDir, type } of CONTENT_DIRS) {
        const contentPath = join(process.cwd(), contentDir);
        const slugs = getSlugs(contentPath).slice(0, limit);

        console.log(`Found ${slugs.length} ${type} entries to process`);

        for (const slug of slugs) {
            const typeDir = join(outputPath, type);
            fs.mkdirSync(typeDir, { recursive: true });

            const outputFile = join(typeDir, `${slug}.png`);

            if (fs.existsSync(outputFile)) {
                console.log(
                    `Skipping ${type}/${slug} - social card already exists`,
                );
                continue;
            }

            const url = `${BASE_URL}/open-graph?type=${type}&slug=${slug}`;

            try {
                console.log(`Generating social card for ${type}/${slug}`);
                const page = await browser.newPage();
                await page.setViewport({
                    width: CARD_WIDTH,
                    height: CARD_HEIGHT,
                    deviceScaleFactor: 2,
                });

                await page.goto(url);
                await timeoutFn(TIMEOUT);

                await page.screenshot({
                    path: outputFile,
                    clip: {
                        x: 0,
                        y: 0,
                        width: CARD_WIDTH,
                        height: CARD_HEIGHT,
                    },
                });

                console.log(
                    `Successfully generated social card for ${type}/${slug}`,
                );
                await page.close();
            } catch (error) {
                console.error(
                    `Failed to generate social card for ${type}/${slug}:`,
                    error,
                );
            }
        }
    }

    await browser.close();
    console.log('Finished generating social cards');
};

generateSocialCards().catch(console.error);
